...
> This is a **simple example** and can be expanded to include more complex schemas.
...
perform full-text searches. This Astro project will be using [SSR][astro-ssr] to handle form submissions and to
...
---
// . . .
import { type PostSearchQuery, PostSeachQuerySchema } from 'db/config.ts';
...
To this point, we have setup full-text search in our Astro project for **local development**. We need to account for
pushing this FTS5 Virtual Table to production and populating it with data. For this, let's create a new seed file:
`seed.prod.ts`.

This file will be used to create the FTS5 Virtual Table if it does not exist and upsert data.

```ts title="db/seed.prod.ts"
import { getCollection } from 'astro:content';
import { sql, db } from 'astro:db';
import { type PostSearchQuery, PostSeachQuerySchema } from './config.ts';
import { fromMarkdown } from 'mdast-util-from-markdown';
import { toString } from 'mdast-util-to-string';

export default async function () {
  const posts = (await getCollection('posts')).map((post) => sql`(${post.slug}, ${post.data.title}, ${post.body})`);

  await db.batch([
    db.run(sql`create virtual table if not exists PostSearch using fts5(slug unindexed, title unindexed, content)`),
    db.run(sql`
      with target(slug, content) as (values ${sql.join(posts, sql`, `)})
      update PostSearch
    ...
    db.run(sql`
      ...
      select slug, content from target
