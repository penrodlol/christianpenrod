    ...
    ```ts title="db/seed.ts"
      ...
      export default async function () {
        ...
        await db.batch([
          ...
          db.run(sql.join([sql`insert into PostSearch(slug, title body) values `, sql.join(posts, sql`,`)])),
        ]);
      }
    ```
...
Now that we have our FTS5 Virtual Table setup, we can create a page that will allow us to perform full-text searches.
This Astro project will be using [SSR][astro-ssr] to handle form submissions and to re-render the page with search
results.

We can create a simple search form with submission validation via [Zod][zod]. Within the Zod schema, we wrap the raw
query with double quotes to allow special characters. Without this, SQLite will throw an error when trying to perform a
full-text search.
...
With the search form in place, we can now perform a full-text search against the `PostSearch` FTS5 Virtual Table:

<Stepper.Root>
  <Stepper.Step>
    Some Zod schemas can be created to validate and provide type-safety for the response from the database. We
    check the validity of the query string and if it passes, we can proceed with the full-text search using
    the `MATCH` operator:

    > The `highlight` function (_[Section 5.1.2][fts5]_) is **optional**. It wraps matched text in the response
    > with HTML tags. For this example, we are using `<mark>` tags which will highlight the text.

    ```astro title="pages/index.astro"
    ---
    // . . .
    const rowSchema = z.object({ slug: z.string(), title: z.string(), content: z.string() });
    const rowsSchema = z.object({ rows: z.array(rowSchema) }).transform((v) => v.rows);

    if (query) {
      const safeQuery = querySchema.safeParse(query);

      if (safeParse.success) {
        const payload = await rowsSchema.safeParseAsync(
          await db.run(
            sql`
  ...
  <Stepper.Step>
    Finally, we can render the search results on the page:
    ...
    <ul>
      {
        results.map((result) => (
          <li>
            <a href={`/posts/${result.slug}`}>{result.title}</a>
            <p>{result.content}</p>
          </li>
        ))
      }
