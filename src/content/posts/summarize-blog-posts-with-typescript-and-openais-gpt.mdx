...
OpenAI's GPT has a max number of [tokens][openai-tokens] that can be passed into
it. This means we must truncate our content to fit within this limit.
Thankfully, we can leverage [gpt-3-encoder][gpt-3-encoder] to do all the heavy
lifting.
